...
const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'papers-061-native-'));
...
async function helloState() {
  try { const last = await json(`http://127.0.0.1:${PORT}/sloptop-picker/hello`); return last?.state ?? null; } catch { return null; }
}
...
async function waitForVisuals(predicate, timeout = 15000) {
  ...
  await waitFor(async () => {
    ...
    return last?.visuals ? predicate(last.visuals) : false;
  }, timeout);
...
async function main() {
  ...
  // Target A (60,60 400x300), Target B (320,160 400x300); A and B overlap in x:[320,460] y:[160,360].
  const targetA = { move: { x: 150, y: 120 } };
  const targetB = { move: { x: 500, y: 220 } };

  // purple hover on unselected target A (bounded, not full-screen)
  ...
  record('physical first target purple hover', firstMove.ok === true && firstHover.state?.visuals?.some((v) => v.kind === 'purple' && isBounded(v)), JSON.stringify(firstHover.state));
  await input('left-click');
  ...
  const firstGreenGeoms = distinctGeometries(firstGreen.state?.visuals ?? [], 'green');
  ...
  // select overlapping target B so A+B both green (two distinct green geometries)
  ...
  record('physical second target (B) purple hover', secondMove.ok === true && secondHover.state?.visuals?.some((v) => v.kind === 'purple' && isBounded(v)), JSON.stringify(secondHover.state));
  await input('left-click');
  ...
  const overlapGreens = distinctGeometries(overlapState.state?.visuals ?? [], 'green');
  record('two distinct overlapping green borders retained (A+B)', overlapGreens.length >= 2, JSON.stringify(overlapState.state));

  // red hover on selected target A (deselect affordance)
  ...
  const postDeselectGreens = distinctGeometries(deselect.state?.visuals ?? [], 'green');
