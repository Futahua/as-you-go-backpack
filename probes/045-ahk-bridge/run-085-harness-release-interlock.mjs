...
const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'papers-085-interlock-'));
...
async function main() {
  ...
  // === WAVE 085: three sequential member clicks with interlocked left-button release ===
  ...
  const clickSequence = [
    { x: 200, y: 800, expect: { x: 60, y: 700 }, label: 'target2' },
    { x: 200, y: 120, expect: { x: 60, y: 60 }, label: 'target0' },
    { x: 200, y: 440, expect: { x: 60, y: 220 }, label: 'target1' },
  ];
  const clickProofs = [];
  let expectedGreen = 0;
  ...
  fs.writeFileSync(path.join(artifacts, 'interlock-proof.json'), JSON.stringify({ runToken, layoutId, token, clickProofs, snapshotAtEscape, cancelLine, afterEscapeHello }, null, 2));
}
