...
const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'papers-060-native-'));
...
async function waitForHello(timeout = 4000) {
...
async function readSharedTrace() {
  return fs.existsSync(sharedTrace) ? fs.readFileSync(sharedTrace, 'utf8') : '';
}
...
async function main() {
  ...
  // Unambiguous target geometry: window A (60,60 400x300), window B (320,160 400x300), window C (60,480 400x300).
  // A and B overlap in x:[320,460] y:[160,360]; click points are each inside only one window.
  const targetA = { x: 150, y: 120, move: { x: 150, y: 120 } };
  const targetB = { x: 500, y: 220, move: { x: 500, y: 220 } };
  const targetC = { x: 150, y: 560, move: { x: 150, y: 560 } };

  // Hold Ctrl+Shift for the whole session; hover POSTs only fire while held.
  await input('ctrl-shift-down');

  // purple hover on unselected target A
  const firstMove = await movePhysical(targetA.move.x, targetA.move.y);
  const firstHover = await waitForVisuals((visuals) => visuals.some((v) => v.kind === 'purple'));
  record('physical first target purple hover', firstMove.ok === true && firstHover.state?.visuals?.some((v) => v.kind === 'purple'), JSON.stringify(firstHover.state));
  await input('left-click');
  const clickDispatch = await waitForTraceContains('event=click', 15000);
  const firstGreen = await waitForVisuals((visuals) => visuals.some((v) => v.kind === 'green'));
  ...
  record('physical first target green persistent selection', firstGreen.state?.visuals?.some((v) => v.kind === 'green'), JSON.stringify(firstGreen.state));

  // select overlapping target B so A+B both green (distinct overlapping green borders)
  const secondMove = await movePhysical(targetB.move.x, targetB.move.y);
  const secondHover = await waitForVisuals((visuals) => visuals.some((v) => v.kind === 'purple'));
  record('physical second target (B) purple hover', secondMove.ok === true && secondHover.state?.visuals?.some((v) => v.kind === 'purple'), JSON.stringify(secondHover.state));
  await input('left-click');
  await waitForTraceContains('event=click', 15000);
  const overlapState = await waitForVisuals((visuals) => visuals.filter((v) => v.kind === 'green').length >= 2);
  const overlapGreens = (overlapState.state?.visuals ?? []).filter((v) => v.kind === 'green');
  record('distinct overlapping green borders retained (A+B >=2)', overlapGreens.length >= 2, JSON.stringify(overlapState.state));

  // red hover on selected target A (deselect affordance) and click deselection
  ...
  record('faint red hover on selected target A', redMove.ok === true && redHover.state?.visuals?.some((v) => v.kind === 'red'), JSON.stringify(redHover.state));
  await input('left-click');
  await waitForTraceContains('event=click', 15000);
  const deselect = await waitForVisuals((visuals) => visuals.filter((v) => v.kind === 'green').length === 1, 15000);
  record('click on selected target A deselects it (green count drops to 1)', deselect.state?.visuals?.filter((v) => v.kind === 'green').length === 1, JSON.stringify(deselect.state));
  ...
  // separate Escape path proves zero mutation
  ...
  fs.writeFileSync(path.join(artifacts, 'identity.json'), JSON.stringify({ dataDir, projectCopy, electron: ELECTRON, ahk: AHK, ahkScript: AHK_SCRIPT, runToken, sharedTrace, childPid, elevatedChildPid, papersPid: papers.proc.pid, targets: targets.map((target) => ({ pid: target.pid, title: target.title })) }, null, 2));
}
try { await main(); } catch (error) { record('bridge runner', false, String(error)); } finally {
  await input('ctrl-shift-up').catch(() => undefined);
  if (elevatedChildPid) {
    spawn('powershell.exe', ['-NoProfile', '-Command', `Stop-Process -Id ${elevatedChildPid} -Force -ErrorAction SilentlyContinue`], { windowsHide: true, stdio: 'ignore' });
  }
  ...
  await sleep(1500);
  fs.writeFileSync(path.join(artifacts, 'cleanup.json'), JSON.stringify({ elevatedChildPid, papersPid: papers?.proc?.pid ?? null, targetPids: targets.map((target) => target.pid), childStillAlive: elevatedChildPid ? await processAlive(elevatedChildPid).catch(() => 'unknown') : null, cleanedAt: new Date().toISOString() }, null, 2));
