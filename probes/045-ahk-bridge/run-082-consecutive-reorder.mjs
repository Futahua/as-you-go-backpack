...
const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'papers-082-reorder-'));
...
async function main() {
  ...
  // Current proven production path: OS receipt + clean-left-button + per-click green waits.
  const p1 = await assertActivationPoint(bp, 'member-add OS');
  if (!p1) return;
  await input('ctrl-shift-up'); await sleep(100);
  await movePhysical(p1.x, p1.y);
  await input('ctrl-shift-down');
  if (!(await releaseLeftButton('member-add pre-activation'))) return;
  ...
  const memberTargets = [{ x: 200, y: 120 }, { x: 200, y: 440 }, { x: 200, y: 800 }];
  let expectedGreen = 0;
  for (const t of memberTargets) {
    ...
    if (!(await releaseLeftButton(`before select member ${expectedGreen + 1}`))) return;
    await input('left-click');
